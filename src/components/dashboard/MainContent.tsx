...
export const MainContent: React.FC<MainContentProps> = ({ isFullscreen }) => {
    ...
    <div className={`transition-all duration-300 ${isFullscreen ? 'lg:ml-0' : 'lg:ml-0'}`}>
      <div className="p-4 sm:p-6 lg:p-8">
        <div className={`max-w-7xl mx-auto space-y-6 transition-all duration-300 ${isDrawerOpen ? 'lg:mr-[400px]' : ''}`}>
          ...
          <div className="bg-black/30 backdrop-blur-sm border border-red-500/20 rounded-2xl p-4 sm:p-6">
            <div className="flex items-center space-x-3 mb-4">
              <div className="p-2 bg-red-500/10 rounded-lg">
                <FileVideo className="h-5 w-5 sm:h-6 sm:w-6 text-red-400" />
              </div>
              <h2 className="text-lg sm:text-xl font-bold text-white">SmartScript AI</h2>
            </div>
          ...
          {/* Steps Progress */}
          <div className="flex justify-center px-4 overflow-x-auto scrollbar-thin scrollbar-track-red-900/20 scrollbar-thumb-red-500/20 pb-2">
                  ...
                  >
                    ...
                    `}>
                      {currentStep > step.number ? (
                        ...
                        <Circle className="w-4 h-4 sm:w-5 sm:h-5 group-hover:scale-110 transition-transform duration-300" />
                      )}
                    </div>
                    <div className="text-center mt-2">
                      <p className="text-xs sm:text-sm font-medium text-white whitespace-nowrap group-hover:text-red-400 transition-colors duration-300">{step.title}</p>
                      <p className="text-[10px] sm:text-xs text-gray-400 whitespace-nowrap group-hover:text-gray-300 transition-colors duration-300">{step.description}</p>
                    </div>
                  </div>
                  {index < steps.length - 1 && (
                    <div className={`
                      w-8 sm:w-12 h-0.5 mt-4 sm:mt-5
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="w-full lg:w-1/2 space-y-4">
              <div className="bg-black/30 backdrop-blur-sm border border-red-500/20 rounded-2xl p-4">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <div className="p-2 bg-red-500/10 rounded-lg">
                      <ListFilter className="h-5 w-5 text-red-400" />
                    </div>
                    <h3 className="text-lg font-bold text-white">Video List</h3>
                  </div>
                  <span className="text-sm text-gray-400">{videoInputs.length}/10 videos</span>
                </div>

                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-track-red-900/20 scrollbar-thumb-red-500/20">
                  {videoInputs.map((input, index) => (
                    <div
                      key={input.id}
                      id={`video-input-${input.id}`}
                      className={`
                        bg-black/20 backdrop-blur-sm border rounded-xl transition-all duration-300
                          ...
                          >
                            ...
                            <div className="relative flex-1">
                              ...
                              <input
                                ...
                                className="w-full bg-black/30 border border-red-500/20 rounded-lg pl-10 pr-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-red-500/50 transition-all duration-300 text-sm"
                              />
                            </div>
                          </button>
                        ...
                        {input.status === 'completed' && (
                          <div className="mt-2 flex items-center justify-between">
                            ...
                            <button
                              ...
                              className="text-xs text-red-400 hover:text-red-300 transition-colors duration-300 flex items-center space-x-1"
                            >
                ...
                {videoInputs.length < 10 && (
                  <button
                    ...
                    className="w-full mt-4 bg-black/20 border border-red-500/20 rounded-xl p-3 text-red-400 hover:text-red-300 hover:border-red-500/40 transition-all duration-300 flex items-center justify-center space-x-2"
                  >
                    <Plus className="h-4 w-4" />
                    <span className="text-sm">Add Another Video</span>
                  </button>
                )}
                ...
                <div className="flex flex-col space-y-3 mt-4">
                  <button
                    ...
                    className={`
                      px-6 py-3 rounded-xl flex items-center justify-center space-x-2 text-sm font-medium
                      ${isProcessing
                  ...
                  {videoInputs.every(input => input.status === 'completed') && (
                    <button
                      ...
                      className={`
                        relative px-6 py-3 bg-gradient-to-r from-red-600 to-red-800 text-white rounded-xl 
            ...
            <div className="w-full lg:w-1/2 space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                    ...
                    <div className="flex bg-black/30 rounded-lg p-1 self-stretch sm:self-auto">
                      <button
                        ...
                        className={`flex-1 sm:flex-none px-4 py-2 rounded-lg text-sm transition-all duration-300 ${
                          activeTab === 'short'
              ...
              <div className="bg-black/30 backdrop-blur-sm border border-red-500/20 rounded-2xl p-4">
                <div className="flex items-center space-x-3 mb-4">
                  ...
                  <h3 className="text-lg font-bold text-white">Custom Instructions</h3>
                </div>
                <div className="relative">
                  <textarea
                    ...
                    className="w-full bg-black/30 border border-red-500/20 rounded-lg p-3 text-white placeholder-gray-500 focus:outline-none focus:border-red-500/50 transition-all duration-300 text-sm min-h-[100px] resize-y"
                  />
                </div>
              </div>
